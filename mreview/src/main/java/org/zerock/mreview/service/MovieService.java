...
public interface MovieService {
    ...
    default MovieDTO entitiesToDTO(Movie movie, List<MovieImage> movieImages, Double avg, Long revieCnt){
        ...
        movieDTO.setAvg(avg);
        movieDTO.setReviewCnt(revieCnt.intValue());
    ...
    default Map<String, Object> dtoToEntity(MovieDTO movieDTO){
        ...
        Movie movie=Movie.builder().mno(movieDTO.getMno()).title(movieDTO.getTitle()).build(); // 빌더 패턴으로 movie객체 생성
        entityMap.put("movie", movie);

        List<MovieImageDTO> imageDTOList=movieDTO.getImageDTOList();

        if(imageDTOList != null && imageDTOList.size() > 0 ) { //MovieImageDTO 처리. size가 0보다 크다는 소리는 list안에 무언가 들어있다는 소리
            ...
            List<MovieImage> movieImageList = imageDTOList.stream().map(movieImageDTO ->{
                ...
                return movieImage;
            }).collect(Collectors.toList());  // collect()함수를 통해 List에 하나씩 넣기
